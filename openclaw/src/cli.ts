...
function cmdDashboard(days: number): void {
  ...
  try {
    filepath = generateDashboard(days);
  } catch (err) {
  ...
  const openFailed = (err: Error | null): void => {
  ...
  if (process.platform === "win32") {
    ...
    execFile(rundll32, ["url.dll,FileProtocolHandler", filepath], { windowsHide: true }, openFailed);
    return;
  }
  const opener = process.platform === "darwin" ? "open" : "xdg-open";
  execFile(opener, [filepath], { windowsHide: true }, openFailed);
}
