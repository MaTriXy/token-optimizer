...
# --------------------------------------------------------------------------- #
# PostToolUse failure fallback for hosts without PostToolUseFailure support
# --------------------------------------------------------------------------- #


def test_posttooluse_failure_fallback_detects_failed_bash_from_tool_response_str(
    monkeypatch, tmp_path, capsys,
):
    """On hosts that do not fire PostToolUseFailure (Cowork, Codex), a failed
    Bash call fires PostToolUse with the error in tool_response. The fallback
    must extract the exit code and run the thrash guard."""
    runner = _load_runner(monkeypatch, tmp_path)
    _no_real_deadline(monkeypatch, runner)
    _record_subcommands(monkeypatch, runner)
    ...
    def _fake_thrash_check(command, stdout, stderr, exit_code):
        nudge_calls.append({"command": command, "exit_code": exit_code})
        return "Stop repeating the same failing command"

    # Stub the thrash_guard import inside the fallback handler
    import sys as _sys
    scripts_dir = str(REPO / "skills" / "token-optimizer" / "scripts")
    if scripts_dir not in _sys.path:
        _sys.path.insert(0, scripts_dir)
    import thrash_guard
    monkeypatch.setattr(thrash_guard, "check", _fake_thrash_check)
    ...
    rc = runner.main()
    assert rc == 0
    assert len(nudge_calls) == 1, "thrash guard must run on failed Bash"
    assert nudge_calls[0]["exit_code"] == 2
    assert nudge_calls[0]["command"] == "make test"

    out = capsys.readouterr().out
    assert "Stop repeating" in out, "thrash guard nudge must reach stdout"


def test_posttooluse_failure_fallback_detects_failed_bash_from_tool_response_dict(
    monkeypatch, tmp_path, capsys,
):
    """The fallback must also handle tool_response as a dict with exit_code."""
    runner = _load_runner(monkeypatch, tmp_path)
    _no_real_deadline(monkeypatch, runner)
    _record_subcommands(monkeypatch, runner)
    ...
    monkeypatch.setattr(runner, "_read_hook_input", lambda: {
        ...
        "tool_response": {"exit_code": 1, "stdout": "build failed"},
        "session_id": "sess-fallback-2",
    })

    rc = runner.main()
    assert rc == 0
    assert len(nudge_calls) == 1, "thrash guard must run on failed Bash (dict)"
    assert nudge_calls[0]["exit_code"] == 1


def test_posttooluse_failure_fallback_skips_successful_bash(
    monkeypatch, tmp_path, capsys,
):
    """The fallback must NOT run the thrash guard on a successful Bash call."""
    runner = _load_runner(monkeypatch, tmp_path)
    _no_real_deadline(monkeypatch, runner)
    _record_subcommands(monkeypatch, runner)
    ...
    nudge_calls = []
    def _fake_thrash_check(command, stdout, stderr, exit_code):
        nudge_calls.append({"exit_code": exit_code})
        return None
    ...
    rc = runner.main()
    assert rc == 0
    assert len(nudge_calls) == 0, "thrash guard must NOT run on successful Bash"


def test_posttooluse_failure_fallback_skips_non_bash_tools(
    monkeypatch, tmp_path, capsys,
):
    """The fallback must only run for Bash, not other tools."""
    runner = _load_runner(monkeypatch, tmp_path)
    _no_real_deadline(monkeypatch, runner)
    _record_subcommands(monkeypatch, runner)
    ...
    monkeypatch.setattr(runner, "_read_hook_input", lambda: {
        ...
        "tool_response": {"exit_code": 1, "stdout": "error"},
        "session_id": "sess-fallback-4",
    })

    rc = runner.main()
    assert rc == 0
    assert len(nudge_calls) == 0, "thrash guard fallback must only run for Bash"
