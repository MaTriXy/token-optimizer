...
|---|---|---|---|
| `TOKEN_OPTIMIZER_CHECKPOINT_RELEVANCE_THRESHOLD` | 0.25 | 0.1–0.9 | Minimum score for the pull tool to resume a checkpoint. Lower widens recall (risks vague-continue false matches); higher demands a stronger topical/resume signal. |
...
| `TOKEN_OPTIMIZER_DASHBOARD_HOOK_BUDGET` | 20 seconds | Wall-clock budget for a dashboard rebuild that runs inside a hook (Stop / SessionEnd). A rebuild the budget stops is finished by a detached background process, so raising this only changes how much of the work happens inside the hook itself. Clamped to 1..600 |
