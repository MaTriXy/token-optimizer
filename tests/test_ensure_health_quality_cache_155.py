...
# --- QA regression: non-string `command` values must not raise ---
#
# A hand-edited settings.json can carry a non-string, non-falsy `command` (a
# number, list, or dict). `_command_drives_quality_cache` used to coerce with
# `blob = command or ""`, which leaves a truthy non-string value untouched, so
# `marker in blob` raised TypeError instead of returning False. Fixed by
# coercing with `str(command or "")`, matching the sibling
# `_quality_cache_hook_present` helper, which already stringifies via `str(group)`.

def test_command_helper_handles_non_string_command_without_raising():
    assert measure._command_drives_quality_cache(123) is False
    assert measure._command_drives_quality_cache([1, 2, 3]) is False
    assert measure._command_drives_quality_cache({"a": 1}) is False


def test_is_quality_bar_installed_handles_non_string_command_without_raising(monkeypatch, tmp_path):
    # Isolate from the real ~/.claude/plugins/cache fallback scan (see
    # test_plugin_cache_fallback_recognizes_shipped_hook above) -- otherwise a
    # real installed dispatcher hook on the machine running the test would
    # make `result["hook"]` True for reasons unrelated to this test.
    ...
    settings = {
        "hooks": {
            "UserPromptSubmit": [
                ...
                {"hooks": [{"type": "command", "command": [1, 2, 3]}]},
                {"hooks": [{"type": "command", "command": {"a": 1}}]},
            ]
        }
    }
    result = measure._is_quality_bar_installed(settings)
