...
As of v5.12.0 (**structure map v2**), the skeleton is smarter about what it keeps. It builds a one-hop reference graph so the map shows what calls a symbol and what a change would touch, ranks symbols by reference-graph centrality (PageRank) so truncation keeps the most-connected ones instead of the first few in source order, and covers 20+ languages through an optional tree-sitter backend. Python and JS/TS work without tree-sitter; installing it extends the same skeletons to Go, Rust, Ruby, Java, C/C++, C#, PHP, and more. The reference graph is resolved at read time and nothing is persisted.

Both of those act on *re-reads*. A related feature, the **first-read retarget**, applies on the *first* read of a large **code** file that history shows is unlikely to be edited soon. In v2 it serves the file you actually opened **in full** — the target is never thinned — and moves the compression budget to skeletons of the *related* files around it, injected as `additionalContext` (cache-safe, never modifying your existing context). The full original of any skeletonized periphery file stays archived and retrievable via `expand`, a ranged Read, or a direct Edit. It stays **code only** — markdown and other prose are served in full, because a headings-only outline drops load-bearing prose — and is gated on real edit-rate proof per file cohort, visible and toggleable in `measure.py v5 status` (`TOKEN_OPTIMIZER_FIRST_READ_ACTIVE` to stop serving while keeping measurement, `TOKEN_OPTIMIZER_FIRST_READ_SHADOW` to disable entirely), and documented with its promotion and tripwire rules on the [Benchmarks page](/reference/benchmarks/#first-read-skeleton-promotion).
