...
def test_extract_session_id_from_prompt(measure):
    ...
    assert f("resume session aaaa1111") == "aaaa1111"
...
def test_explicit_id_cross_project_is_declined(measure, monkeypatch):
    ...
    recency branches enforce."""
    mod, cp_dir = measure
    _write_checkpoint(cp_dir, _UUID_A, _sidecar(_UUID_A, "secret client work"), age_minutes=5)
    # Simulate the named session belonging to a DIFFERENT project (override the
    # fixture's always-in-project stub).
    monkeypatch.setattr(mod, "_checkpoint_in_project", lambda sc, cwd: False, raising=True)
    cps = mod.list_checkpoints()
    block = mod._continuity_resume_block(
        f"continue session {_UUID_A}", cps, None, "/home/u/other-project")
    assert block == ""
    assert "secret client work" not in block
...
def test_bare_uuid_uses_conditional_footer(measure):
    ...
    cps = mod.list_checkpoints()
    block = mod._continuity_resume_block(
        f"the failing test references {_UUID_A} somewhere", cps, None, "/home/u/proj")
    assert "the alpha task" in block
    assert _CONDITIONAL in block
    assert _CONFIDENT not in block
